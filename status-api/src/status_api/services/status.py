...
class Status:
    ...
    def check_status(cls, service_name: str, check_date: arrow.Arrow):
        ...
        response = {"service": service_name, "current_status": current_status, "next_up_time": up_time}
        ...
        if service_schedule:
            ...
            if current_status == "True":
                if "available" in service_schedule[0]:
                    ...
                    for available in available_times:
                        if check_date_local.date() == available[0].date() and (
                            check_date_local < available[0] or check_date_local > available[1]
                        ):
            ...
            if custom is not None:
                if (
                    Status._get_local_outage_time(custom["start"])
                    <= check_date_local
                ...
                for day_week in week_dates:
                    if day_week.isoweekday() == int(available_time["dayofweek"]):
                        ...
                        date_availables.append(
                            [
                                day_week.replace(hour=from_hours, minute=from_minutes),
