...
export const useBcrosAccount = defineStore('bcros/account', () => {
  ...
  const authConfig = {
    headers: { Authorization: `Bearer ${sessionStorage.getItem(SessionStorageKeyE.KEYCLOAK_TOKEN)}` }
  }
  ...
  async function getAuthUserProfile (identifier: string) {
    return await useBcrosFetch<KCUserI>(`${apiURL}/users/${identifier}`, authConfig)
      .then(({ data, error }) => {
  ...
  async function getUserAccounts (keycloakGuid: string) {
    return await useBcrosFetch<UserSettingsI[]>(`${apiURL}/users/${keycloakGuid}/settings`, authConfig)
      .then(({ data, error }) => {
        if (error.value || !data.value) {
