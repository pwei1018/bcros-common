...
from colin_report_migration.services.gcp_auth.auth_service import GoogleAuthService
from colin_report_migration.services.utils.exceptions import StorageException
...
class GoogleStorageService:  # pylint: disable=too-few-public-methods
    ...
    def save_document(cls, name: str, raw_data):
        ...
        try:
            ...
            return cls.__call_cs_api(HTTP_POST, name, raw_data, CONTENT_TYPE_PDF)
        except StorageException as storage_err:
            raise storage_err
        except Exception as err:  # pylint: disable=broad-except # noqa F841;
    ...
    @classmethod
    def save_document_link(cls, name: str, raw_data, available_days: int = 1):
        """Save a document to a cloud storage bucket with the binary data as the file contents. Return a link."""
        try:
            logger.info(f"Saving document name={name}.")
            return cls.__call_cs_api_link(name, raw_data, available_days, CONTENT_TYPE_PDF)
        except StorageException as storage_err:
            raise storage_err
        except Exception as err:  # pylint: disable=broad-except # noqa F841;
