...
"""Tests to verify the endpoints for maintaining MHR documents.

Test-Suite to ensure that the /mhr endpoint is working as expected.
"""
...
def test_get(session, client, jwt, desc, params, roles, account, doc_class, status):
    ...
    if status == HTTPStatus.OK:  # Create.
        ...
        with open(TEST_DATAFILE, 'rb') as data_file:
            ...
            data_file.close()
        response = client.post('/api/v1/mhr/MHR_MISC' + PARAMS1,
                               data=raw_data,
                               headers=headers,
...
def test_create(session, client, jwt, desc, content_type, roles, account, doc_class, doc_type, status):
    """Assert that a post save new MHR document works as expected."""
    # setup
    current_app.config.update(AUTH_SVC_URL=MOCK_AUTH_URL)
    headers = None
    json_data = {}
