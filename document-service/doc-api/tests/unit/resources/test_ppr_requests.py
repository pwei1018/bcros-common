...
"""Tests to verify the endpoints for maintaining PPR documents.

Test-Suite to ensure that the /ppr endpoint is working as expected.
"""
...
DOC_CLASS1 = DocumentClasses.PPR.value
DOC_TYPE1 = DocumentTypes.PPR_MISC.value
...
PATH: str = "/api/v1/ppr/{doc_type}" + PARAMS1
...
def test_get(session, client, jwt, desc, params, roles, account, doc_class, status):
    """Assert that a get PPR documents request works as expected."""
    # setup
    ...
    if status == HTTPStatus.OK:  # Create.
        ...
        with open(TEST_DATAFILE, "rb") as data_file:
            ...
            data_file.close()
        response = client.post("/api/v1/ppr/PPR_MISC" + PARAMS1, data=raw_data, headers=headers, content_type=MEDIA_PDF)
...
def test_create(session, client, jwt, desc, content_type, roles, account, doc_class, doc_type, status):
    """Assert that a post save new PPR document works as expected."""
    # setup
    current_app.config.update(AUTH_SVC_URL=MOCK_AUTH_URL)
    headers = None
    json_data = {}
