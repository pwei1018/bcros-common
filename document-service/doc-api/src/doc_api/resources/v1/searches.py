...
bp = Blueprint('SEARCHES1',  # pylint: disable=invalid-name
               __name__, url_prefix='/searches')


@bp.route('/<string:doc_class>', methods=['GET', 'OPTIONS'])
@jwt.requires_auth
def get_searches(doc_class: str):
    """Get document information for records that match the query parameters."""
    try:
        req_path: str = GET_REQUEST_PATH.format(doc_class=doc_class)

        info: RequestInfo = RequestInfo(RequestTypes.GET,
                                        req_path,
                                        None,
