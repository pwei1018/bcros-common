...
export const useBcrosLaunchdarkly = defineStore('bcros/launchdarkly', () => {
  ...
  function init () {
    ...
    let user: object = { key: 'anonymous', appSource: appName }
    let org: object = { key: 'anonymous', appName }
  ...
  function getFeatureFlag (name: string): string|null {
    return ldClient.value ? ldClient.value.variation(name) : null
  }

  function getStoredFlag (name: string): string {
    if (!ldInitialized) {
