...
class TestOpsHealthEndpoint:
    ...
    def test_healthz_with_sqlalchemy_error(self, session, client):
    ...
    def test_healthz_with_database_connection_error(self, session, client):
    ...
    def test_healthz_with_database_timeout_error(self, session, client):
    ...
    def test_healthz_with_generic_exception(self, session, client):
    ...
    def test_healthz_with_runtime_error(self, session, client):
    ...
    def test_healthz_unsupported_methods(self, session, client, method):
...
class TestOpsReadyEndpoint:
    ...
    def test_readyz_success(self, client):
    ...
    def test_readyz_no_database_dependency(self, client):
    ...
    @pytest.mark.parametrize("method", ["POST", "PUT", "DELETE", "PATCH"])
    def test_readyz_unsupported_methods(self, client, method):
        """Assert that unsupported HTTP methods return 405."""
        response = client.open("/ops/readyz", method=method)
        assert response.status_code == HTTPStatus.METHOD_NOT_ALLOWED

    def test_readyz_multiple_requests(self, client):
...
class TestOpsEndpointsIntegration:
    ...
    def test_invalid_ops_endpoints(self, client):
