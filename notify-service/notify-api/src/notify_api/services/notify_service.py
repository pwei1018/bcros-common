...
class NotifyService:
    ...
    def get_provider(cls, request_by: str, content_body: str, notification_request: NotificationRequest = None) -> str:
        """Get the notify service provider based on request source and content type.
        ...
        """
        if not request_by or not isinstance(request_by, str):
            logger.warning("Invalid request_by parameter, defaulting to GC_NOTIFY")
            return Notification.NotificationProvider.GC_NOTIFY

        if request_by.upper() == STRR_REQUEST_IDENTIFIER:
            # Send email through GC Notify Housing service
            logger.debug(f"Using HOUSING provider for {request_by} request")
            return Notification.NotificationProvider.HOUSING

        # Check if total attachment size exceeds 6MB
        if notification_request:
            try:
                if cls._has_large_attachments(notification_request):
                    logger.debug("Using SMTP provider for large attachments (>6MB)")
                    return Notification.NotificationProvider.SMTP
            except Exception as e:
                logger.error(f"Error calculating attachment size, defaulting to SMTP: {e}")
                return Notification.NotificationProvider.SMTP

        # Send email through SMTP if email body contains HTML
        if content_body and cls._contains_html(content_body):
            logger.debug("Using SMTP provider for HTML content")
            return Notification.NotificationProvider.SMTP

        logger.debug("Using GC_NOTIFY provider for plain text content")
        return Notification.NotificationProvider.GC_NOTIFY
    ...
    def _get_delivery_topic(self, provider: str) -> str | None:
    ...
    def _filter_safe_recipients(self, recipients: str) -> list[str]:
        """Filter recipients based on safe list in development environment.
        ...
        safe_recipients = [r.strip() for r in recipients.split(",") if SafeList.is_in_safe_list(r.lower().strip())]

        unsafe_recipients = [
            r.strip() for r in recipients.split(",") if r.strip().lower() not in [sr.lower() for sr in safe_recipients]
        ]

        if unsafe_recipients:
            logger.info(f"Recipients not in safe list: {unsafe_recipients}")

        return safe_recipients

    def _create_cloud_event(self, provider: str, notification_data: dict) -> SimpleCloudEvent:
    ...
    def _update_notification_status(self, notification: Notification, provider: str, status: str) -> None:
    ...
    def queue_publish(self, notification_request: NotificationRequest) -> Notification:
        ...
        try:
            ...
            safe_recipients = self._filter_safe_recipients(notification_request.recipients)
            ...
            delivery_topic = self._get_delivery_topic(provider)
            ...
            for recipient in safe_recipients:
                ...
                if self._process_single_recipient(
    ...
    def _process_single_recipient(
        ...
        try:
            ...
            notification = Notification.create_notification(notification_request, recipient)
            notification_data["notificationId"] = notification.id

            # Create and publish cloud event
            cloud_event = self._create_cloud_event(provider, notification_data)
            ...
            self._update_notification_status(notification, provider, Notification.NotificationStatus.QUEUED)
            ...
            try:
                if "notification" in locals():
                    self._update_notification_status(notification, provider, Notification.NotificationStatus.FAILURE)
            except Exception as update_err:
                logger.error(f"Failed to update notification status for {recipient}: {update_err}")

            return False

    def queue_republish(self) -> None:
        ...
        try:
            ...
            for notification in notifications:
                if self._republish_single_notification(notification):
                    successful_count += 1
                else:
                    failed_count += 1
    ...
    def _republish_single_notification(self, notification: Notification) -> bool:
        ...
        try:
            # Get delivery topic for the provider
            delivery_topic = self._get_delivery_topic(notification.provider_code)
            if not delivery_topic:
                logger.error(
            ...
            # Create and publish cloud event
            cloud_event = self._create_cloud_event(notification.provider_code, republish_data)
            publish_future = queue.publish(delivery_topic, GcpQueue.to_queue_message(cloud_event))
