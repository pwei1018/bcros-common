...
"""Test suite for BC Notify Housing resource handlers."""
...
from notify_delivery.resources.bc_notify_housing import bp, worker
from notify_delivery.services.providers.bc_notify_housing import BCNotifyHousing


class TestBCNotifyHousingResource(unittest.TestCase):
    """Test suite for BC Notify Housing resource handlers."""
    ...
    @patch("notify_delivery.resources.bc_notify_housing.get_cloud_event")
    @patch("notify_delivery.resources.bc_notify_housing.validate_event_type")
    @patch("notify_delivery.resources.bc_notify_housing.process_notification")
    @patch("notify_delivery.resources.bc_notify_housing.logger")
    def test_worker_valid_bc_notify_housing_event(self, mock_logger, mock_process, mock_validate, mock_get_event):
        """Worker returns 200 and processes valid BC Notify Housing event."""
        mock_ce = Mock()
        mock_ce.type = "bc.registry.notify.bc_notify_housing"
        ...
        assert response == {}
        mock_process.assert_called_once_with(mock_ce.data, BCNotifyHousing)
    ...
    def test_blueprint_registration(self):
        ...
        assert bp.name == "bc-notify-housing"
