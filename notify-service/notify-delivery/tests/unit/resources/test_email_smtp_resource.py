...
"""Test suite for email SMTP resource handlers."""
...
from notify_delivery.resources.email_smtp import bp, process_message, worker


class TestEmailSMTPResource(unittest.TestCase):
    """Test suite for email SMTP resource handlers."""
    ...
    @patch("notify_delivery.resources.email_smtp.ensure_authorized_queue_user")
    @patch("notify_delivery.resources.email_smtp.queue")
    @patch("notify_delivery.resources.email_smtp.logger")
    def test_worker_no_request_data(self, mock_logger, mock_queue, mock_auth):
    ...
    @patch("notify_delivery.resources.email_smtp.queue")
    @patch("notify_delivery.resources.email_smtp.logger")
    def test_worker_no_cloud_event(self, mock_logger, mock_queue, mock_auth):
    ...
    @patch("notify_delivery.resources.email_smtp.logger")
    def test_worker_valid_smtp_event(self, mock_logger, mock_process, mock_queue, mock_auth):
        """Test worker endpoint with valid SMTP cloud event."""
        # Arrange
        mock_auth.return_value = lambda f: f  # Mock decorator
        mock_ce = Mock()
        mock_ce.type = "bc.registry.notify.smtp"
    ...
    @patch("notify_delivery.resources.email_smtp.queue")
    @patch("notify_delivery.resources.email_smtp.logger")
    def test_worker_invalid_event_type(self, mock_logger, mock_queue, mock_auth):
        ...
        mock_logger.error.assert_called_with(
            "Invalid queue message type: expected 'bc.registry.notify.smtp', got 'invalid.type'"
        )
    ...
    @patch("notify_delivery.resources.email_smtp.process_message")
    @patch("notify_delivery.resources.email_smtp.logger")
    def test_worker_processing_exception(self, mock_logger, mock_process, mock_queue, mock_auth):
    ...
    @patch("notify_delivery.resources.email_smtp.Notification")
    @patch("notify_delivery.resources.email_smtp.EmailSMTP")
    @patch("notify_delivery.resources.email_smtp.NotificationHistory")
    def test_process_message_successful_send(self, mock_history, mock_smtp, mock_notification):
        ...
        mock_smtp_instance.send.return_value = mock_responses
        ...
        mock_smtp.assert_called_once_with(mock_notification_obj)
        mock_smtp_instance.send.assert_called_once()
        # Check that status was set to SENT (mocked object will have status_code attribute set)
    ...
    @patch("notify_delivery.resources.email_smtp.Notification")
    @patch("notify_delivery.resources.email_smtp.EmailSMTP")
    def test_process_message_failed_send(self, mock_smtp, mock_notification):
        ...
        mock_smtp_instance = Mock()
        mock_smtp_instance.send.return_value = None  # Failed send
        mock_smtp.return_value = mock_smtp_instance
        ...
        mock_smtp.assert_called_once_with(mock_notification_obj)
        mock_smtp_instance.send.assert_called_once()
        # Check that status was set to FAILURE (mocked object will have status_code attribute set)
        assert hasattr(mock_notification_obj, "status_code")
        mock_notification_obj.update_notification.assert_called_once()
    ...
    @patch("notify_delivery.resources.email_smtp.EmailSMTP")
    @patch("notify_delivery.resources.email_smtp.NotificationHistory")
    def test_process_message_multiple_recipients(self, mock_history, mock_smtp, mock_notification):
