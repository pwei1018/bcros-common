...
"""Database connection helpers shared across the notify-delivery service.

These helpers mirror the equivalents in the notify-api ``utils.util`` module.
They are kept local to notify-delivery so the service stays self-contained and
does not depend on a newer (possibly unpublished) notify-api revision being
pinned by the git dependency.
"""
